...
@section('content')
    <section class="content-header">
        ...
        <div class="card card-primary">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form action="{{ str_replace('http://', 'https://', secure_url(route('category.store'))) }}" method="post">
                @csrf
                <div class="card-body">
                    <div class="form-row">
                        <!-- Name -->
                        ...
                        <div class="col-lg-6 col-md-6 mb-3">
                            ...
                            <input type="text" value="{{ old('discount') }}" name="discount" min="0" max="100"
                                class="form-control" placeholder="{{ __('Enter Discount') }}">
                        </div>
                       
    
                        <div class="col-lg-6 col-md-6 mb-3">
                            <label>{{ __('Starts From') }}</label>
                            <input type="text" value="{{ old('starts_from ') }}" name="starts_from" 
                                class="form-control" placeholder="{{ __('Enter Starts From') }}">
                        </div>
                        <div class="col-lg-6 col-md-6 mb-3">
                            <label>{{ __('Order') }}</label>
                            <input type="number" value="{{ old('ordr') }}" name="ordr" 
                            ...
                            <select name="parent_id" class="form-control">
                                ...
                                @foreach ($parentCategory as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('parent_id') == $category->parent_id ? 'selected' : '' }}>
